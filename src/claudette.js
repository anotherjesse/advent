...
class Client {
    ...
    async call(msgs, { sp = '', temp = 0, maxTokens = 4096, stream = false, prefill = '' } = {}) {
        msgs = mkMsgs(msgs);
...
class Chat {
    ...
    async call(pr = null, { temp = 0, maxTokens = 4096, stream = false, prefill = '' } = {}) {
        if (pr && this.h.length > 0 && this.h[this.h.length - 1].role === 'user') {
            await this.call();
        }
        pr && this.h.push(mkMsg({ content: pr }));

        const result = await this.c.call(this.h, { sp: this.sp, temp, maxTokens, stream, prefill });
